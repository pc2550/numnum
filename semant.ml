                        ...
                        (match(e) with
                            | Id(m) -> (match (type_of_identifier m) with
                                | Matrix(t, l) ->
                                    ...
                                    in
                                    ignore(List.fold_left comp_matrix e actuals); raise(Failure ("Come on!"))
                                | _ -> raise (Failure ("illegal argument to " ^ fname ^ " found expected Matrix in " ^ (string_of_expr e))))
                            | _ -> raise(Failure ("illegal argument to " ^ fname ^ " found expected Matrix in "^ (string_of_expr e))) 
                        )
                    | _ -> raise(Failure ("illegal argument to " ^ fname ^ " found in "^ (string_of_expr e1))) 
                )
         | (Binop (e1, op, e2) as e) ->
              let t1 = expr e1
              and t2 = expr e2
