...
numnum.native :
	ocamlbuild -use-ocamlfind -pkgs llvm,llvm.analysis,llvm.bitwriter,llvm.bitreader,llvm.linker,llvm.target -cflags -w,+a-4 \
		numnum.native
...
codegen.cmo : ast.cmo
codegen.cmx : ast.cmx
numnum.cmo : semant.cmo scanner.cmo parser.cmi codegen.cmo ast.cmo
numnum.cmx : semant.cmx scanner.cmx parser.cmx codegen.cmx ast.cmx
...
numnum-llvm.tar.gz : $(TARFILES)
	cd .. && tar czf numnum-llvm/numnum-llvm.tar.gz \
		$(TARFILES:%=numnum-llvm/%)
